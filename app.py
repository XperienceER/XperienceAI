...
def chat():
    ...
    if model_name in NVIDIA_MODELS:
        ...
        try:
            ...
            # บางโมเดล thinking ส่ง reasoning แยกมา ถ้า content ว่างให้ใช้ reasoning แทน
    ...
    # ไม่ส่ง last_error ออกไป กัน leak รายละเอียดภายใน (log ไว้ฝั่ง server แทน)
    print(f"[ERROR] OpenRouter keys failed: {last_error}")
    return jsonify({"error": "API Key ใช้งานไม่ได้ทุกตัว กรุณาลองใหม่ภายหลัง"}), 500
